...
class ClientCompat:
    ...
    def record(self, event, properties={}, timestamp=None,
               uri=KISSmetrics.RECORD_URI):
        return self.client.record(person=self.identity, event=event,
                                  properties=properties, timestamp=timestamp,
                                  uri=uri)

    def set(self, properties={}, timestamp=None, uri=KISSmetrics.SET_URI):
        return self.client.set(person=self.identity, properties=properties,
                               timestamp=timestamp, uri=uri)

    def alias(self, identity, uri=KISSmetrics.ALIAS_URI):
        return self.client.alias(person=self.identity, identity=identity,
                                 uri=uri)
